...
class DigitalTwinHandler(Handler):
    ...
    def execute(self, node: Node, context: Context, graph: Graph,
        ...
        # Safely get checkpoint_dir from config kwarg
        config = kwargs.get("config")
        project_dir = config.checkpoint_dir if config else ""
        if not project_dir:
            return Outcome(
                status=StageStatus.FAIL,
                failure_reason="Missing checkpoint_dir for Digital Twin metadata"
            )
        ...
        twin_data = {
            ...
            "nodes_completed": list(context.snapshot().keys()),
